...
// ─── Skill: 简历优化（合并版）───
const RESUME_OPTIMIZE_PROMPT = `你是简历优化专家。根据候选人背景、目标岗位 JD 和现有经历，完成两件事：
1. 按优先级给出 5-8 条针对性改写建议（每条有 before/after）
2. 生成一份针对该岗位定制的完整简历
...
返回 JSON：
{
  ...
  "suggestions": [
    {
      "id": 1,
      "title": "改写建议标题（如：技能段补充 Python 项目细节）",
      "impact": "+3pp",
      "tags": ["关键词+2"],
      "original": "原始描述",
      "improved": "优化后的描述（用 STAR 法则，量化成果）",
      "reason": "为什么这样改（结合 JD 要求说明）"
    }
  ],
  "resume": {
    "sections": [
      {"title": "求职意向", "content": "岗位 | 城市"},
      {"title": "教育背景", "content": "学校 · 专业 · 学历\\n相关课程：..."},
      {"title": "核心技能", "content": "按 JD 匹配度排序，用 / 分隔"},
      {"title": "项目经历", "content": "用 STAR 法则重写"},
      {"title": "实习经历", "content": "用 STAR 法则重写"},
...
规则：
- originalScore 是当前简历对该岗位的匹配度(0-100)
- optimizedScore 是优化后的预估匹配度(0-100)
- suggestions 按影响分数（impact）降序排列，impact 格式为 "+Npp"
- tags 是该建议涉及的优化维度（如 "关键词+2"/"匹配度+5"/"ATS优化"）
- 每条 suggestion 的 original 和 improved 要具体，不要泛泛而谈
- resume.sections 的内容必须基于候选人真实背景，不能凭空捏造
- 技能栏按 JD 关键词匹配度降序排列
- 每段经历用 STAR 法则，有量化数据
只返回 JSON。`;
