  ...
  return (
    <div className="relative">
      <button
        ...
        className={`w-8 h-8 rounded-full flex items-center justify-center transition border ${enabled ? "text-brand-500 bg-brand-50 border-brand-200" : "text-[var(--text-t)] hover:text-brand-500 border-[var(--border)] hover:border-brand-500 hover:bg-brand-50"}`}
        title="邮件推送设置"
      >
      ...
      {open && (
        <div className="absolute right-0 top-10 w-72 card p-4 z-50 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold text-[var(--text)]">每日岗位推送</span>
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="accent-brand-500" />
              <span className="text-xs text-[var(--text-s)]">{enabled ? "开" : "关"}</span>
            </label>
          </div>
          {enabled && (
            <input
          ...
          <button onClick={save} className="w-full py-2 rounded-[var(--radius-xs)] text-xs font-semibold text-white bg-brand-500 hover:bg-brand-500 transition">
            {saved ? "已保存 ✓" : "保存"}
          </button>
          <p className="text-[10px] text-[var(--text-t)]">每天早上推送与你画像匹配的新增岗位，可随时关闭。</p>
        </div>
      )}
    </div>
  );
...
function ThemeToggle() {
  ...
  return (
    <button onClick={toggle} className="w-8 h-8 rounded-full flex items-center justify-center border border-[var(--border)] text-[var(--text-t)] hover:text-brand-500 hover:border-brand-500 hover:bg-brand-50 transition" title={dark ? "切换亮色" : "切换暗色"}>
      {dark ? "☀" : "☾"}
    </button>
  );
}
    ...
    <><header className="sticky top-0 z-40 bg-[var(--surface)] backdrop-blur-[8px] [backdrop-filter:blur(8px)_saturate(180%)] [-webkit-backdrop-filter:blur(8px)_saturate(180%)] border-b border-[var(--border)]" style={{ boxShadow: "0 1px 8px rgba(91,76,255,.04)" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        <a href="/" className="flex items-center gap-2.5 group">
          <div className="w-7 h-7 rounded-[var(--radius-xs)] bg-gradient-to-br from-brand-400 to-brand-500 grid place-items-center shadow-[var(--shadow-sm)] group-hover:shadow-[var(--shadow-md)] transition">
            <span className="text-xs font-black text-white">C</span>
          </div>
          <span className="text-lg font-black text-[var(--text)] group-hover:text-brand-500 transition tracking-tight">Career Search</span>
        </a>
        ...
        <nav className="hidden md:flex items-center gap-1.5 text-sm">
          {navItems.map((item) => {
            ...
            return (
              <a key={item.label} href={item.href} className={`px-3.5 py-1.5 rounded-full text-[13px] font-semibold transition ${active ? "bg-brand-500 text-white shadow-[var(--shadow-sm)]" : item.highlight ? "text-brand-500 bg-brand-50" : "text-[var(--text-s)] hover:text-brand-500 hover:bg-brand-50"}`}>{item.label}</a>
            );
          })}
          ...
          {loggedIn && user ? (
            <button
              ...
              className="w-8 h-8 rounded-full overflow-hidden ring-2 ring-white shadow-[var(--shadow-sm)] hover:ring-brand-200 transition ml-1"
            ...
            <button onClick={() => openLogin()} className="text-[13px] font-semibold text-white px-4 py-1.5 rounded-full bg-brand-500 hover:bg-brand-500 shadow-[var(--shadow-sm)] transition ml-1">登录</button>
          )}
        </nav>
        ...
        <div className="flex md:hidden items-center gap-2">
          <ThemeToggle />
          <button onClick={() => setMenuOpen(!menuOpen)} className="w-8 h-8 rounded-full flex items-center justify-center text-[var(--text-s)] border border-[var(--border)] hover:border-brand-500 hover:text-brand-500">
            {menuOpen ? "✕" : "☰"}
          </button>
        </div>
      </div>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-[var(--border)] bg-[var(--surface)] backdrop-blur-[8px] [backdrop-filter:blur(8px)_saturate(180%)] px-4 py-3 space-y-1">
          {navItems.map((item) => {
            const active = isActive(item.href);
            return (
              <a key={item.label} href={item.href} className={`block py-2.5 px-3 rounded-[var(--radius-xs)] text-sm font-semibold transition ${active ? "bg-brand-500 text-white" : "text-[var(--text)] hover:text-brand-500 hover:bg-brand-50"}`} onClick={() => setMenuOpen(false)}>{item.label}</a>
            );
          })}
          {loggedIn ? (
      ...
      {showLogin && (
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center" onClick={() => setShowLogin(false)}>
          <div className="bg-[var(--surface-solid)] rounded-[var(--radius)] border border-[var(--border)] shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4" onClick={(e) => e.stopPropagation()} style={{ boxShadow: "0 24px 80px rgba(91,76,255,.12), 0 8px 32px rgba(0,0,0,.08)" }}>
            <h2 className="text-lg font-bold text-[var(--text)] text-center">登录</h2>
            ...
            {emailSent ? (
              <div className="text-center space-y-3 py-2">
                ...
                <p className="text-sm text-[var(--text)]">验证码已发送至</p>
                <p className="text-sm font-semibold text-[var(--text)]">{loginEmail}</p>
                <input
                  type="text"
                  inputMode="numeric"
                  value={otpCode}
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-[var(--radius-xs)] bg-brand-500 text-white text-sm font-semibold hover:bg-brand-500 disabled:opacity-50 transition"
                >
                  {verifying ? "验证中..." : "验证登录"}
                </button>
                ...
                <div className="flex items-center justify-center gap-3">
                  <button
                    ...
                    disabled={cooldown > 0}
                    className="text-xs text-brand-500 hover:text-brand-500 disabled:text-[var(--text-t)]"
                  >
                  ...
                  <button
                    ...
                    className="text-xs text-brand-500 hover:text-brand-500"
                  >
                    使用其他邮箱
                  </button>
                </div>
              ...
              <div className="space-y-3">
                ...
                <p className="text-center text-[11px] text-[var(--text-t)]">无需注册，输入邮箱即可收到验证码</p>
              </div>
            )}

            <div className="relative">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[var(--border)]" /></div>
              <div className="relative flex justify-center text-xs"><span className="bg-[var(--surface-solid)] px-2 text-[var(--text-t)]">或</span></div>
            </div>

            <button
              onClick={handleGitHubAuth}
              className="w-full py-2.5 rounded-[var(--radius-xs)] border border-[var(--border-s)] text-sm font-semibold text-[var(--text)] hover:bg-brand-50 hover:text-brand-500 hover:border-brand-500 transition flex items-center justify-center gap-2"
            >
            ...
            <p className="text-center text-[10px] text-[var(--text-t)]">GitHub 登录需要科学上网</p>
          </div>
        </div>
      )}
    </>
